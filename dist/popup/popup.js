...
document.addEventListener('DOMContentLoaded', () => {
    ...
    getElement('add-bookmark-form').addEventListener('submit', handleAddBookmark);
...
function loadSettings() {
    chrome.storage.sync.get(['bookmarks', 'searchEngine', 'theme'], (data) => {
        ...
        if (data.searchEngine) {
            const searchEngineInput = document.querySelector(`input[name="search-engine"][value="${data.searchEngine}"]`);
        ...
        if (data.theme) {
            ...
            // 默认主题
            const defaultThemeButton = document.querySelector('.theme-option[data-theme="light"]');
...
function hideAddBookmarkDialog() {
    ...
    getElement('add-bookmark-form').reset();
}
// 处理添加书签
...
function saveSettings() {
    // 获取搜索引擎设置
    const searchEngineInput = getElementBySelector('input[name="search-engine"]:checked');
    const searchEngine = searchEngineInput.value;
    // 获取主题设置
    const themeElement = getElementBySelector('.theme-option.active');
